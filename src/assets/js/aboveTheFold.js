  ...
  var fNum = function() {
    if (num.length > 0) {
      ...
      num.parent().each(function() {
        ...
        if (!self.hasClass('target') || Modernizr.touchevents) {
    ...
    if (histEvent.length > 0) {
      histEvent.each(function() {
        ...
        if (!self.hasClass('target') || Modernizr.touchevents) {
          var winTop = $(window).scrollTop(),
            topPos = self.offset().top - $(window).height();
          if ((winTop >= topPos) || Modernizr.touchevents) {
            self.addClass('target');
            self.delay(defaults.speedAnimation / 3).animate({
              'opacity': 1
      ...
      chartEvent.each(function() {
        ...
        if (!self.hasClass('target') || Modernizr.touchevents) {
          ...
          if (winTop >= topPos || Modernizr.touchevents) {
            ...
            self.easyPieChart({
              barColor: '#e7543d',
              animate: (!Modernizr.touchevents) ? defaults.speedAnimation * 4 : -1,
              size: 210,
              lineWidth: 12,
              lineCap: 'butt',
